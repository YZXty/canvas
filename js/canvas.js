$(function(){
    ...
    * */
    ...
    $(".option:last input").eq(0).change(function(){
        canvasObj.xpsize=$(this).val();
    });
    $(".option:last input").eq(1).click(function(){
        $(".xp").css("display","none");
        canvasObj.aaa=0;
        canvasObj.draw()
    })
    ...
    $(".back").click(function(){
         ...
         else if(canvasObj.histroy.length==1){
                    canvasObj.histroy.pop();
                    cobj.clearRect(0,0,canvas.width,canvas.height);
         }else{
             canvasObj.histroy.pop();
             cobj.putImageData(canvasObj.histroy[canvasObj.histroy.length-1],0,0);
         }
    })
    ...
    $(".save").click(function(){
        if(canvasObj.histroy.length>0) {
            location.href = canvas.toDataURL("image/png").replace("image/png", "stream/octet");
        }
    })
